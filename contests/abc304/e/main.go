...
func solve() {
	...
	for i := 0; i < k; i++ {
		...
		conditions.Add(Condition{x: x, y: y})
	}
	...
	for i := 0; i < q; i++ {
		...
		if uf.isSame(p, q) {
			...
			if conditions.Has(Condition{x: p, y: q}) {
				fmt.Println("No")
			} else {
				fmt.Println("Yes")
			}
...
func (uf *UnionFind) copy() *UnionFind {
    newParents := make([]int, len(uf.parents))
    copy(newParents, uf.parents)

    return &UnionFind{parents: newParents}
}
