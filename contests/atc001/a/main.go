...
func solve() {
	...
	dfs = func(y, x int) bool {
		visited.Add(Item{y, x})
		if y == goal.y && x == goal.x {
			return true
		}
		nexts := []Item{(Item{y - 1, x}), (Item{y + 1, x}), (Item{y, x - 1}), (Item{y, x + 1})}
		for _, next := range nexts {
			nextY, nextX := next.y, next.x
			if nextY < 0 || nextY >= h || nextX < 0 || nextX >= w || grid[nextY][nextX] == "#"  || visited.Has(Item{nextY, nextX}) {
				continue
			}
			if dfs(nextY, nextX) {
				return true
