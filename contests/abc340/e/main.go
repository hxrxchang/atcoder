...
func solve() {
	...
	segtree := NewLazySegmentTree(n, 0, func(a, b int) int { return a + b }, func(a, b int) int { return a + b })
	for i := 0; i < m; i++ {
		items := a[b[i]-1]
		segtree.UpdateRange(0, n, items/n)
		remainder := items % n
		segtree.UpdateRange(0, remainder, 1)
	}
}

// LazySegmentTree represents a segment tree with lazy propagation.
type LazySegmentTree[T any] struct {
	data     []T     // Segment Tree data
	lazy     []T     // Lazy propagation array
	lazyFlag []bool  // Indicates if lazy data needs to be applied
	n        int     // Number of leaves (total number of elements)
	e        T       // Identity element for the operation
	op       func(T, T) T // Segment Tree operation
	lazyOp   func(T, T) T // Operation to apply lazy updates
}

// NewLazySegmentTree creates a new Lazy Segment Tree.
func NewLazySegmentTree[T any](n int, e T, op, lazyOp func(T, T) T) *LazySegmentTree[T] {
