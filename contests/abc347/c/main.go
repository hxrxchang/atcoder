...
func solve() {
	...
	schedules := getInts()
	for i := 0; i < n; i++ {
		schedules[i] = schedules[i] % (a + b)
	}
	schedules = sortSlice(schedules)

	for i := 0; i < n; i++ {
		schedules = append(schedules, schedules[i] + a + b)
	}

	for i := 0; i <= n; i++ {
		segment := schedules[i:i+n]
		if segment[len(segment)-1] - segment[0] < a {
			fmt.Println("Yes")
			return
		}
	}
