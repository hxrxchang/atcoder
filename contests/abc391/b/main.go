...
func solve() {
	...
	gridS := make([][]string, n)
	for i := 0; i < n; i++ {
		gridS[i] = strToSlice(getStr(), "")
	}

	gridT := make([][]string, m)
	for i := 0; i < m; i++ {
		gridT[i] = strToSlice(getStr(), "")
	}
	...
	for i := 0; i < n; i++  {
		for j := 0; j < n; j++ {
			...
			gridS2 := make([][]string, m)
			for k := i; k < i + m; k++ {
				gridS2[k - i] = gridS[k][j:j+m]
				if equal2DSlices(gridS2, gridT) {
					fmt.Println(i+1, j+1)
					return
				}
			}
...
func equal2DSlices[T comparable](a, b [][]T) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
