...
func solve() {
	...
	que := newQueue[Item]()
	que.PushFront(Item{height: BIGGEST, idx: 0})

	res := make([]int, n+1)
	for i := 1; i <= n; i++ {
		for que.Size() > 0 && que.Front().height <= h[i] {
			que.PopFront()
		}
		pre := que.Front()
		res[i] = (i - pre.idx) * h[i] + res[pre.idx]
		que.PushFront(Item{height: h[i], idx: i})
	}
