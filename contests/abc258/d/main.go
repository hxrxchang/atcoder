...
func solve() {
	...
	for i := 0; i < n; i++ {
		in := getInts()
		a, b := in[0], in[1]
		stages[i] = Stage{story: a, play: b}
	}

	type Result struct {
		stageCnt, totalCost, min int
	}
	results := make([]Result, n)
	results[0] = Result{1, stages[0].story + stages[0].play, stages[0].play}
	for i := 1; i < n; i++ {
		stage := stages[i]
		total := stage.story + stage.play
		cost := results[i-1].totalCost + total
		minCost := min(results[i-1].min, stage.play)
		results[i] = Result{results[i-1].stageCnt + 1, cost, minCost}
	}

	ans := BIGGEST
	for i := 0; i < n; i++ {
		needCnt := x - results[i].stageCnt
		tmp := results[i].totalCost + needCnt * results[i].min
		if tmp < ans {
			ans = tmp
		}
	}
