...
func solve() {
	...
	reachableFromBlackNode := make([]bool, n)
	...
	for i := 0; i < q; i++ {
		...
		typ, v := in[0], in[1]-1
		if typ == 1 {
			if !reachableFromBlackNode[v] {
				reachableFromBlackNode[v] = true
				que := newQueue[int]()
				que.PushBack(v)
				for que.Size() > 0 {
					v2 := que.PopFront()
					for _, next := range graph[v2] {
						if !reachableFromBlackNode[next] {
							reachableFromBlackNode[next] = true
							que.PushBack(next)
						}
					}
				}
