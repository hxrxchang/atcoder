...
func solve() {
	...
	sumT := sum(t)
	// dp[i]: その時点での料理を作るときの、i分にするのに必要な最小の時間
	dp := make([]int, sumT + 1)
	for i := 0; i < len(dp); i++ {
		dp[i] = BIGGEST
	}
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < len(dp); j++ {
			// 配るDP
			if j + t[i] < len(dp) {
				tmp[j + t[i]] = min(tmp[j + t[i]], dp[j] + t[i])
			}
		}
		dp = tmp
	}

	// 2つの電子レンジを並列で使うので、2つの料理を作る最小時間は時間がかかる方の電子レンジの最小時間になる。
	// よって、全料理作るのにかかる合計時間の半分以上の中で最小の時間を求めればいい
	ans := min(dp[ceilDiv(sumT, 2):]...)
