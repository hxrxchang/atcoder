...
func solve() {
	...
	for {
		candidates := enumerateValidIdCandidates(s, 3, 16)
		...
		if !nextPermutation(sort.StringSlice(s)) {
			break
		}
	}
...
func enumerateValidIdCandidates(words []string, minLen, maxLen int) []string {
	var result []string
	n := len(words)

	var dfs func(current []string, index int, currentLen int)
	dfs = func(current []string, index int, currentLen int) {
		if index == n {
		...
		remainingMinLen := 0
		for i := index; i < n; i++ {
			remainingMinLen += len(words[i]) + 1
		}

		if currentLen+remainingMinLen > maxLen {
			return
		}

		for underscoreCount := 1; currentLen+len(words[index])+underscoreCount <= maxLen; underscoreCount++ {
			underscore := strings.Repeat("_", underscoreCount)
			dfs(append(current, underscore+words[index]), index+1, currentLen+len(words[index])+underscoreCount)
		}
	}


	dfs([]string{words[0]}, 1, len(words[0]))
	return result
}
