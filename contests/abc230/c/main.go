...
func solve() {
	in := getInts()
	_, a, b := in[0], in[1], in[2]
	in = getInts()
	p, q, r, s := in[0], in[1], in[2], in[3]

	grid := make([][]string, q-p+1)
	for i := 0; i <= q-p; i++ {
		row := make([]string, s-r+1)
		for j := 0; j <= s-r; j++ {
			row[j] = "."
		}
		grid[i] = row
	...
	for i := 0; i <= q - p; i++ {
		for j := 0; j <= s - r; j++ {
			i2 := i + p
			j2 := j + r
			// 黒く塗れる範囲は a+k==i2 かつ b+k==j2となる箇所
			// 式変形すると、k==i2-a, k==j2-b となるので、i2-a==j2-bとなる箇所が対象になる
			if i2 - a == j2 - b {
				grid[i][j] = "#"
			}
			// 黒く塗れる範囲は a+k==i2 かつ b-k==j2となる箇所
			// 式変形すると、k==i2-a, k==b-j2 となるので、i2-a==b-j2となる箇所が対象になる
			if i2 - a == b - j2 {
				grid[i][j] = "#"
			}
		}
