...
func solve() {
	...
	subsetsH := generateSubsets(rangeSlice(h1))
	subsetsW := generateSubsets(rangeSlice(w1))

	for _, subsetH := range subsetsH {
		for _, subsetW := range subsetsW {
			tmpA2 := make([]int, 0)
			for _, vh := range subsetH {
				for _, vw := range subsetW {
					tmpA2 = append(tmpA2, a[vh][vw])
				}
			}
			a2 := make([][]int, len(subsetH))
			for i, v := range tmpA2 {
				a2[i/len(subsetW)] = append(a2[i/len(subsetW)], v)
			}
			if isSame(a2, b) {
				fmt.Println("Yes")
				return
