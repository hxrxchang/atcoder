...
func solve() {
	n := getInt()
	perfetctSquares := make([]int, 0)
	for i := 1; i <= n; i++ {
		if isPerfectSquare(i) {
			perfetctSquares = append(perfetctSquares, i)
		}
	}

	fmt.Println(perfetctSquares)

	lenPerfetctSquares := len(perfetctSquares)
	s := rangeSlice(lenPerfetctSquares)

	ans := n + len(getCombinations(s, 2))
	fmt.Println(ans)
}
