...
func solve() {
	...
	for i := 0; i < q; i++ {
		...
		if in[0] == 1 {
			u, v := in[1]-1, in[2]-1
			u, v = uf.Root(u), uf.Root(v)
			if u == v {
				continue
			}
			if uf.parents[u] > uf.parents[v] {
				u, v = v, u
			}
			nodesByRoot[u] = append(nodesByRoot[u], nodesByRoot[v]...)
			nodesByRoot[u] = descendingSortSlice(nodesByRoot[u])
			nodesByRoot[u] = nodesByRoot[u][:min(10, len(nodesByRoot[u]))]
			nodesByRoot[v] = []int{}
			uf.Unit(u, v)
		} else {
			v, k := in[1]-1, in[2]
			v = uf.Root(v)
			if len(nodesByRoot[v]) < k {
				fmt.Println(-1)
			} else {
				fmt.Println(nodesByRoot[v][k-1]+1)
			}
		}
	}
}
