...
func bitDp(graph [][]int) int {
	...
	for state := 0; state < 1<<n; state++ {
		...
		for i := firstUnSelected+1; i < n; i++ {
			if (state>>i)&1 == 0 {
				secondUnSelected := i
				// 未選択だった頂点 firstUnSelected と secondUnSelected を選ぶことにしたので、対応するビットを立てて新しい状態を作る
				newState := state | (1 << firstUnSelected) | (1 << secondUnSelected)
				// その状態にしたことで得られる重さの最大値を計算
				dp[newState] = max(dp[newState], dp[state]+graph[firstUnSelected][secondUnSelected])
			}
		}
	}
	return dp[(1<<n)-1]
