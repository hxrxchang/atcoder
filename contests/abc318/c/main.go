...
func solve() {
	in := getInts()
	n, d, p := in[0], in[1], in[2]
	f := getInts()
	f = descendingSortSlice(f)

	accmF := make([]int, n)
	accmF[0] = f[0]
	for i := 1; i < n; i++ {
		accmF[i] = accmF[i-1] + f[i]
	}
	...
	for i := 0; i < n; i++ {
		...
		amount := f[i]
		var untilYesterDayAmount int
		if i == 0 {
			untilYesterDayAmount = 0
		} else {
			untilYesterDayAmount = accmF[i-1]
		}
		dDaysTotalAmountFromToday := accmF[min(i+d-1, n-1)] - untilYesterDayAmount
		// 今日からみてd日間の合計金額がより、フリーチケット買ったほうが安い場合
		if p < dDaysTotalAmountFromToday {
			totalAmount += p
			tmp += d - 1
		} else {
			totalAmount += amount
