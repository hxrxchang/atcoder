...
func solve() {
	aIn := getInts()
	a := FloatPoint{float64(aIn[0]), float64(aIn[1])}

	bIn := getInts()
	b := FloatPoint{float64(bIn[0]), float64(bIn[1])}

	cIn := getInts()
	c := FloatPoint{float64(cIn[0]), float64(cIn[1])}

	dIn := getInts()
	d := FloatPoint{float64(dIn[0]), float64(dIn[1])}

	// 一個でも三角形の中に入っていたらNo
	if isPointInTriangle(a, b, c, d) || isPointInTriangle(b, c, d, a) || isPointInTriangle(c, d, a, b) || isPointInTriangle(d, a, b, c) {
		fmt.Println("No")
	} else {
		fmt.Println("Yes")
	}
...
// 4点あるときに、ある点が他の3点で構成される三角形の中に入っているかどうかを判定する
func isPointInTriangle(p, a, b, c FloatPoint) bool {
    // Calculate vectors
    v0 := FloatPoint{c.x - a.x, c.y - a.y}
    v1 := FloatPoint{b.x - a.x, b.y - a.y}
    v2 := FloatPoint{p.x - a.x, p.y - a.y}
