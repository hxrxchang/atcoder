...
func solve() {
	...
	for i := 0; i < n; i++ {
		in := getInts()
		t, x, a := in[0], in[1], in[2]
		snukes[i] = &Snuke{t, x, a}
		maxTime = max(maxTime, t)
	}

	snukes2 := make([][]int, maxTime + 1)
	for i := range snukes2 {
		snukes2[i] = make([]int, HOLES_LEN)
	}
	for _, snuke := range snukes {
		snukes2[snuke.t][snuke.x] = snuke.a
	}

	dp := snukes2[0]
	for time := 1; time <= maxTime; time++ {
		tmp := make([]int, HOLES_LEN)
		copy(tmp, dp)
		for hole := 0; hole < HOLES_LEN; hole++ {
