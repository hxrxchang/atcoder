...
func solve() {
	...
	for i := 0; i < n; i++ {
		in := getInts()
		pieces[i] = Piece{i, in[0], in[1]}
	}

	sortedByHeight := newQueue[Piece]()
	for _, v := range sortSliceFn(pieces, func(a, b Piece) bool {
		return a.h > b.h
	}) {
		sortedByHeight.PushBack(v)
	}

	sortedByWidth := newQueue[Piece]()
	for _, v := range sortSliceFn(pieces, func(a, b Piece) bool {
		return a.w > b.w
	}) {
		sortedByWidth.PushBack(v)
	}
	...
	ans := make([]Ans, n)
	for used.Size() != n {
		tmp := sortedByHeight.Front()
		if h == tmp.h && !used.Has(tmp.idx) {
			v := sortedByHeight.PopFront()
			ans[v.idx] = Ans{h - v.h + 1, w - v.w + 1}
			w -= v.w
			used.Add(v.idx)
		} else {
			v := sortedByWidth.PopFront()
			if used.Has(v.idx) {
				continue
			}
			ans[v.idx] = Ans{h - v.h + 1, w - v.w + 1}
