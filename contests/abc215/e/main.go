...
func solve() {
	...
	for i := 0; i < n; i++ {
		...
		// 文字がAなら0, Bなら1, Cなら2..., Jなら9 となる
		cur := int(s[i][0] - 'A')

		next[1<<cur][cur] = (next[1<<cur][cur] + 1) % MOD
		...
		for bit := 0; bit < (1 << 10); bit++ {
			for last := 0; last < 10; last++ {
				...
				next[bit][last] = (next[bit][last] + val) % MOD

				// 選ぶ場合
				if (bit>>cur)&1 == 0 {
					// まだ使われていない種類なら選べる
					next[bit|(1<<cur)][cur] = (next[bit|(1<<cur)][cur] + val) % MOD
				} else if last == cur {
					// 直前と同じ種類ならOK（ひとかたまり）
					next[bit][cur] = (next[bit][cur] + val) % MOD
				}
				// それ以外（同じ種類だが間に別が入る）はNG
			}
		}
		dp = next
	}
