...
func isOk(accmInterval []int, objectiveCnt, canUseCnt int) bool {
	for i := 0; i + objectiveCnt < len(accmInterval); i++ {
		rng := accmInterval[i:i+objectiveCnt+1]
		if rng[len(rng)-1] - rng[0] <= canUseCnt {
			return true
		}
	}
	return false
