...
func solve() {
	...
	for i := 0; i < n-1; i++ {
		...
		graph[a] = append(graph[a], Edge{to: b, cost: c})
		graph[b] = append(graph[b], Edge{to: a, cost: c})
		edgeCostSum += 2 * c
	}
	...
	// 先に求めたノードから最も遠いノードまでの距離を求める
	// これが木の直径になる
	dist2 := make([]int, n)
	for i := 0; i < n; i++ {
		dist2[i] = -1
	}
	que.PushBack(maxDistNode)
	dist2[maxDistNode] = 0
	maxDist2 := 0
	for que.Size() > 0 {
		current := que.PopFront()
		for _, edge := range graph[current] {
			...
			dist2[edge.to] = dist2[current] + edge.cost
			que.PushBack(edge.to)
			maxDist2 = max(maxDist2, dist2[edge.to])
		}
	}

	ans := edgeCostSum - maxDist2
