...
func solve() {
	...
	for i := 0; i < m; i++ {
		in := getInts()
		x, y, d := in[0]-1, in[1]-1, in[2]
		graph[x] = append(graph[x], DijkstraItem{node: y, dist: d + a[y]})
		graph[y] = append(graph[y], DijkstraItem{node: x, dist: d + a[x]})
	}

	dist := dijkstra(graph, 0)
	// 各頂点への最短経路に始点の頂点重みを加える
	for i := 0; i < n; i++ {
		if dist[i] != BIGGEST {
			dist[i] += a[0]
		}
