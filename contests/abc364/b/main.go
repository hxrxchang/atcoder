...
func solve() {
	...
	for i := 0; i < len(x); i++ {
		if x[i] == "U" {
			nextY := currentY - 1
			if nextY >= 0 && grid[nextY][currentX] == "." {
				currentY = nextY
			}
		} else if x[i] == "D" {
			nextY := currentY + 1
			if nextY <= h && grid[nextY][currentX] == "." {
				currentY = nextY
			}
		} else if x[i] == "L" {
			nextX := currentX - 1
			if nextX >= 0 && grid[currentY][nextX] == "." {
				currentX = nextX
			}
		} else if x[i] == "R" {
			nextX := currentX + 1
			if nextX <= w && grid[currentY][nextX] == "." {
				currentX = nextX
			}
		}
	}

	fmt.Println(currentY + 1, currentX + 1)
}
