...
func solve() {
	in := getInts()
	n, m, x := in[0], in[1], in[2]

	graph1 := make([][]dijkstraItem, n)
	graph2 := make([][]dijkstraItem, n)

	for i := 0; i < m; i++ {
		in := getInts()
		a, b := in[0]-1, in[1]-1
		graph1[a] = append(graph1[a], dijkstraItem{node: b, dist: 1})
		graph2[b] = append(graph2[b], dijkstraItem{node: a, dist: 1})
	}
	...
	minCost := dist1[n-1]
	for i := 1; i < n-1; i++ {
		if dist1[i] != BIGGEST && dist2[i] != BIGGEST {
			minCost = min(minCost, dist1[i]+x+dist2[i])
		}
	}
