...
func solve() {
	...
	nextNodes := make(map[GridBfsNode][]GridBfsNode)
	for i := 0; i < y; i++ {
		for j := 0; j < x; j++ {
			if grid[i][j] == "." {
				for _, d := range []GridBfsNode{{i-1, j}, {i+1, j}, {i, j-1}, {i, j+1}} {
					if d.y >= 0 && d.y < y && d.x >= 0 && d.x < x && grid[d.y][d.x] == "." {
						nextNodes[GridBfsNode{i, j}] = append(nextNodes[GridBfsNode{i, j}], GridBfsNode{d.y, d.x})
					}
				}
			}
		}
	...
	distances := gridBfs(y, x, nextNodes, GridBfsNode{sy, sx})
	fmt.Println(distances[gy][gx])
}
