...
func solve() {
	...
	// sの中で、0が1文字以上連続する区間の先頭を判定したい
	isZeroStart := make([]bool, n)
	for i, v := range s {
		if i == 0 && v == "0" {
			isZeroStart[0] = true
			continue
	...
	// 累積和で0が連続している部分文字列の数を数え上げる
	accm := make([]int, n+1)
	for i, v := range isZeroStart {
		accm[i+1] = accm[i]
		if v {
			accm[i+1]++
		}
...
func isOk(accm []int, s []string, length, limit int) bool {
	for l := 0; l+length < len(accm); l++ {
		r := l + length
		needCnt := accm[r] - accm[l]
		// もしウィンドウの左端が 0 で、かつそれより前が 0 なら
		// その連続区間の先頭は l の外にあるがウィンドウと交差しているので +1する
		if s[l] == "0" && (l > 0 && s[l-1] == "0") {
			needCnt++
		}
		if needCnt <= limit {
			return true
