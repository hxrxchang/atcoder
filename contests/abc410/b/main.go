...
func solve() {
	...
	boxSets := make([]*Set[int], n+1)
	for i := 0; i <= n; i++ {
		boxSets[i] = newSet[int]()
	}
	...
	for i := 0; i < q; i++ {
		...
		if box != 0 {
			boxSets[box].Add(ball)
		} else {
			var minimumBox int
			var minimumSize int = BIGGEST
			for i := 1; i <= n; i++ {
				if boxSets[i].Size() < minimumSize {
					minimumBox = i
					minimumSize = boxSets[i].Size()
				}
			}
			boxSets[minimumBox].Add(ball)
		}
	}
