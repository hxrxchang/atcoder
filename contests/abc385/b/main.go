...
func solve() {
	in := getInts()
	h, w, x, y := in[0], in[1], in[2]-1, in[3]-1
	...
	visited := newSet[GridItem]()

	current := GridItem{x, y}
	t := strToSlice(getStr(), "")
	for _, v := range t {
		if v == "U" {
			if current.x - 1 >= 0 && grid[current.x-1][current.y] != "#" {
				current.x--
			...
			if current.x + 1 < h && grid[current.x+1][current.y] != "#" {
				current.x++
				if grid[current.x][current.y] == "@" {
					visited.Add(current)
				}
			...
			if current.y + 1 < w && grid[current.x][current.y+1] != "#" {
				current.y++
				if grid[current.x][current.y] == "@" {
					visited.Add(current)
				}
	...
	fmt.Println(current.x+1, current.y+1, visited.Size())
}
