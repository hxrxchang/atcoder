...
func solve() {
	...
	heights := make(map[int][]GridItem)
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			heights[grid[y][x]] = append(heights[grid[y][x]], GridItem{y, x})
		}
	}
	...
	for height := 1; height <= y; height++ {
		for _, item := range heights[height] {
			idx := item.y*w + item.x
			uf.unit(idx, seaNode)
			for _, d := range directions {
				ny, nx := item.y+d.dy, item.x+d.dx
		...
		fmt.Println(h * w - uf.size(seaNode) + 1)
	}
}

// 海に面しているかどうかを判定
func check(y, x, h, w int, uf *UnionFind) bool {
	if y == 0 || y == h - 1 || x == 0 || x == w - 1 {
		return true
	}
	return uf.isSame(position2Idx(y - 1, x, w), position2Idx(y, x, w)) || uf.isSame(position2Idx(y + 1, x, w), position2Idx(y, x, w)) ||
	uf.isSame(position2Idx(y, x - 1, w), position2Idx(y, x, w)) || uf.isSame(position2Idx(y, x + 1, w), position2Idx(y, x, w))
}

func position2Idx(y, x, w int) int {
	return y * w + x
}
