...
func solve() {
	...
	for i := 0; i < m; i++ {
		in := getInts()
		operations[i] = Operation{in[0], in[1]-1, in[2]}
	}
	...
	colors := sortSlice(maps.Keys(colorCnt))
	...
	for _, color := range colors {
		if colorCnt[color] > 0 {
			ans = append(ans, Answer{color, colorCnt[color]})
		}
	}

	fmt.Println(len(ans))
	for _, a := range ans {
		fmt.Println(a.color, a.cnt)
	}
}
