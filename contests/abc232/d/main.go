...
func solve() {
	...
	graph := make([][]string, h)
	for i := 0; i < h; i++ {
		graph[i] = strToSlice(getStr(), "")
	}

	nextItems := make(map[GridBfsNode][]GridBfsNode)
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			current := GridBfsNode{i, j}
			if graph[i][j] == "#" {
				continue
			}
			if j + 1 < w && graph[i][j+1] == "." {
				nextItems[current] = append(nextItems[current], GridBfsNode{i, j+1})
			}
			if i + 1 < h && graph[i+1][j] == "." {
				nextItems[current] = append(nextItems[current], GridBfsNode{i+1, j})
			}
		}
	}

	dist := gridBfs(h, w, nextItems, GridBfsNode{0, 0})
