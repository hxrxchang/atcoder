...
func solve() {
	...
	takahashi := make([][]bool, n)
	for i := 0; i < n; i++ {
		takahashi[i] = make([]bool, n)
	}
	for i := 0; i < m; i++ {
		in := getInts()
		a, b := in[0]-1, in[1]-1
	...
	permutations := rangeSlice(n)
	for {
		if check(takahashi, aoki, permutations, n) {
			fmt.Println("Yes")
			return
		}

		if !nextPermutation(sort.IntSlice(permutations)) {
			break
		}
	}
...
func check(takahashi, aoki [][]bool, permutations []int, n int) bool {
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			if takahashi[i][j] != aoki[permutations[i]][permutations[j]] {
				return false
			}
		}
	}
