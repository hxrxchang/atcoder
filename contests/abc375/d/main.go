...
func solve() {
	...
	// 各英大文字ABC...が、sのi文字目までに何回出現するかを累積和でカウント
	mp := make(map[string][]int)
	abc := strToSlice("ABCDEFGHIJKLMNOPQRSTUVWXYZ", "")
	for _, v := range abc {
		mp[v] = []int{0}
	}
	for i, v := range s {
		for _, w := range abc {
			if v == w {
				mp[w] = append(mp[w], mp[w][i]+1)
			} else {
				mp[w] = append(mp[w], mp[w][i])
			}
		}
	}
	...
	for i := range s {
		tmp := 0
		for _, w := range abc {
			tmp += mp[w][i] * (mp[w][len(s)] - mp[w][i+1])
		}
		cnt += tmp
	}
