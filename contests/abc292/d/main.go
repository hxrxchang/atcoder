...
func solve() {
	...
	for i, n := range uf.parents {
		if n > -1 {
			children[n] = append(children[n], i)
		} else {
			children[i] = append(children[i], i)
		}
	}

	flag := true
	for _, p := range maps.Keys(children) {
		nodeCnt := uf.size(p)
		edgeCnt := 0
		for _, c := range children[p] {
			edgeCnt += edgeCntFromNode[c]
		}
