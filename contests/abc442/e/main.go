...
func solve() {
	...
	monsters := make([]float64, n)
	monstersAngles := make([]float64, 0)
	for i := 0; i < n; i++ {
		in := getInts()
		x, y := in[0], in[1]
		angle := math.Ceil(angle(x, y))
		monsters[i] = angle
		monstersAngles = append(monstersAngles, angle)
		monstersAngles = append(monstersAngles, angle+360)
	}

	monstersAngles = sortSlice(monstersAngles)
	...
	for i := 0; i < q; i++ {
		...
		fmt.Println(countInRange(monstersAngles, angleA, angleB+1))
	}
}
