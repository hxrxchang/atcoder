...
func isOk(a []int, rangeCnt, cnt int) bool {
	initialInterval := a[:rangeCnt]
	ms := newMultiset[int]()
	for _, v := range initialInterval {
		ms.Add(v)
	}
	...
	for i := rangeCnt; i < len(a); i++ {
		erace := initialInterval[0]
		ms.Remove(erace)
		initialInterval = initialInterval[1:]

		add := a[i]
		ms.Add(add)
		initialInterval = append(initialInterval, add)
