...
// 羊羹をcutLength以上の長さになるように切ったときの、分割数がblockCnt以上になるかどうか
func check(cutPositions []int, cutLength, blockCnt int) bool {
	cnt := 0
	// 最後にカットした位置
	lastCut := -1
	for i, v := range cutPositions {
		var currentLength int
		if lastCut == -1 {
			currentLength = v
		} else {
			currentLength = v - cutPositions[lastCut]
		}
		// 最低の長さ以上になったら切る
		// 残りの長さもcutLength以上である必要がある
		if currentLength >= cutLength && cutPositions[len(cutPositions)-1] - cutPositions[i] >= cutLength {
			cnt++
			lastCut = i
		}
	}
