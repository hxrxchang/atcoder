...
func solve() {
	...
	k := getInt()
	// kが0の場合、無条件なので適当にどこか一つのNodeを黒に塗って終了
	if k == 0 {
		fmt.Println("Yes")
		res := make([]string, n)
		for i := 0; i < n; i++ {
	...
	conditionKeys := maps.Keys(condition)
	for _, key := range conditionKeys {
		flag := false
		for node, dist := range distances[key] {
			targetDist := condition[key]
			if dist == targetDist && colors[node] != "W" {
				flag = true
				colors[node] = "B"
			}
		}
