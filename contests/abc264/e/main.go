...
func solve() {
	in := getInts()
	n, _, e := in[0], in[1], in[2]
	edges := make([][]int, e)
	for i := 0; i < e; i++ {
		in := getInts()
		// 0-indexedでn移行の頂点は発電所。発電所は同一視する。
		x := min(in[0]-1, n)
		y := min(in[1]-1, n)
		edges[i] = []int{x, y}
	}
	...
	for i := 0; i < e; i++ {
		if !deleted[i] {
			uf.unit(edges[i][0], edges[i][1])
		}
	}

	ans := make([]int, q)
	// 後ろからイベントを見る。見終わったらつなげることで、前から削除するのと同じ結果になる。
	for i := q - 1; i >= 0; i-- {
		ans[i] = uf.size(n) - 1 // 発電所がつながっている都市数。-1は発電所を除くため
		x := events[i]
		uf.unit(edges[x][0], edges[x][1])
	}
