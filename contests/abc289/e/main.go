...
func solve() {
	...
	for i := 0; i < t; i++ {
		...
		// visited[i][j]: 高橋くんがiに、青木くんがjに訪問済みかどうか
		visited := make([][]bool, n)
		for i2 := 0; i2 < n; i2++ {
			visited[i2] = make([]bool, n)
		}

		que := newQueue[BfsItem]()
		que.PushBack(BfsItem{0, n-1, 0})
		visited[0][n-1] = true

		for que.Size() > 0 {
			current := que.PopFront()
			if current.takahashi == n - 1 && current.aoki == 0 {
				result[i] = current.dist
				break
			}
			for _, nextTakahashi := range graph[current.takahashi] {
				for _, nextAoki := range graph[current.aoki] {
					if colors[nextTakahashi] == colors[nextAoki] || visited[nextTakahashi][nextAoki] {
						continue
					}
					que.PushBack(BfsItem{nextTakahashi, nextAoki, current.dist + 1})
					visited[nextTakahashi][nextAoki] = true
				}
			}
		}
	}
