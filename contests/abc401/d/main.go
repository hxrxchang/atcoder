...
func solve() {
	...
	posCnt := make([]PosState, n+1)
	for i := 0; i <= n; i++ {
		posCnt[i] = PosState{dot: 0, o: 0}
	}

	var dfs func(pos int, cnt int, prev string, tmp string)
	dfs = func(pos int, cnt int, prev string, tmp string) {
		if pos == n {
			return
		}
