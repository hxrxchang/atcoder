...
func solve() {
	...
	if n == 1 {
		nodes[a[0]] = Node{left: MINIMUM, right: BIGGEST}
	} else {
		for i := 0; i < n; i++ {
			if i == 0 {
				nodes[a[0]] = Node{left: MINIMUM, right: a[1]}
			} else if i == n-1 {
				nodes[a[n-1]] = Node{left: a[n-2], right: BIGGEST}
			} else {
				nodes[a[i]] = Node{left: a[i-1], right: a[i+1]}
			}
		}
	}
	...
	for i := 0; i < q; i++ {
		...
		if in[0] == 1 {
			...
			// xの後ろにyを挿入
			nodes[y] = Node{left: x, right: tmp.right}
			if _, ok := nodes[tmp.right]; ok {
				nodes[tmp.right] = Node{left: y, right: nodes[tmp.right].right}
			}
			nodes[x] = Node{left: tmp.left, right: y}
		} else {
			x := in[1]
			tmp := nodes[x]

			// xを削除
			if _, ok := nodes[tmp.left]; ok {
				nodes[tmp.left] = Node{left: nodes[tmp.left].left, right: tmp.right}
			}
			if _, ok := nodes[tmp.right]; ok {
				nodes[tmp.right] = Node{left: tmp.left, right: nodes[tmp.right].right}
			}
			delete(nodes, x)
		}
	}
