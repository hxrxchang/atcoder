...
func solve() {
	...
	for i := 0; i < n; i++ {
		people[i] = Person{weight: w[i], isAdult: s[i] == "1"}
	}
	sort.Slice(people, func(a, b int) bool {
		return people[a].weight < people[b].weight
	})

	// dを0とした初期状態でそれぞれの人が合っているかを計算しておく
	// 0未満になることはないので、初期状態で子どもは絶対にいない
	cnt := 0
	...
	for i, v := range people {
		if v.isAdult {
			// 本来大人として扱う人を子ども扱いするので合っている人数から減らす
			cnt--
		} else {
			// 本来子どもとして扱うべき人をちゃんと子どもとして扱えるので、合っている人数に増やす
			cnt++
		}
