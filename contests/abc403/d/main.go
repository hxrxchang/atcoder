...
func solve() {
	...
	graph := make([][]int, n)
	for i := 0; i < n; i++ {
		graph[i] = append(graph[i], mp[a[i]+d]...)
		graph[i] = append(graph[i], mp[a[i]-d]...)
	}
	...
	for i := 0; i < n; i++ {
		...
		target1 := a[i] + d
		target2 := a[i] - d
		// fmt.Println(i, a[i], target1, target2)
		if target2 >= 0 {
			v1 := moreThan(mp[target1], i)
			v2 := moreThan(mp[target2], i)

			if v1 != nil && v2 != nil {
				if len(graph[i]) < min(len(graph[*v1]), len(graph[*v2])) {
				...
				if len(graph[i]) < len(graph[*v2]) {
					deleted.Add(*v2)
				} else {
					cnt++
				}
			} else if v1 != nil {
				if len(graph[i]) < len(graph[*v1]) {
					deleted.Add(*v1)
				} else {
					cnt++
				}
