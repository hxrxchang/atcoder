...
func solve() {
	...
	que := newQueue[Item]()
	que.PushBack(Item{sy, sx, 0})

	for que.Size() > 0 {
		item := que.PopFront()
		y, x, steps := item.y, item.x, item.steps
		if dist[y][x] != -1 {
			continue
		}
		dist[y][x] = steps
		...
		// 上下左右
		for _, next := range [][]int{{item.y - 1, item.x}, {item.y + 1, item.x}, {item.y, item.x - 1}, {item.y, item.x + 1}} {
			ny, nx := next[0], next[1]
			...
			que.PushFront(Item{ny, nx, steps})
		}
		...
		for i := -2; i <= 2; i++ {
			for j := -2; j <= 2; j++ {
				...
				que.PushBack(Item{y + i, x + j, steps + 1})
			}
		}
	}
