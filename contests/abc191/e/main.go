...
func solve() {
	...
	for i := 0; i < m; i++ {
		...
		graph[a] = append(graph[a], dijkstraItem{b, c})
	}

	distCache := make([][]int, n)

	for i := 0; i < n; i++ {
		distCache[i] = dijkstra(graph, i)
	}
	...
	for i := 0; i < n; i++ {
		...
		for j, d := range dist {
			if i == j || d == BIGGEST || distCache[j][i] == BIGGEST {
				continue
			}

			ans = min(ans, d + distCache[j][i])
		}
