...
func solve() {
	...
	for i := 0; i < n; i++ {
		in := getInts()
		services[i] = Service{in[0], in[1], in[2]}
		days = append(days, in[0], in[1]+1)
	}

	zaatsu := newZaatsu(days)
	imosu := make([]int, zaatsu.Count())

	for _, s := range services {
		imosu[zaatsu.GetCompressedValue(s.start)] += s.cost
		imosu[zaatsu.GetCompressedValue(s.end+1)] -= s.cost
	}

	for i := 1; i < len(imosu); i++ {
		imosu[i] += imosu[i-1]
	}

	ans := 0
	for i := 0; i < len(imosu)-1; i++ {
		interval := zaatsu.GetOriginalValue(i+1) - zaatsu.GetOriginalValue(i)
		ans += min(imosu[i], c) * interval
	}
