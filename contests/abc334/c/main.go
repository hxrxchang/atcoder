...
func solve() {
	...
	if k % 2 == 0 {
		ans := 0
		for i := 0; i < k; i+=2 {
			ans += remains[i+1]-remains[i]
		}
		fmt.Println(ans)
		return
	}

	accmLeft := make([]int, k+1)
	for i := 1; i <= k; i++ {
		accmLeft[i] = accmLeft[i-1]
		ai := i-1
		if i % 2 == 0 {
			accmLeft[i] += remains[ai] - remains[ai-1]
		}
	}

	accmRight := make([]int, k+1)
	for i := k-2; i >= 0; i-- {
		accmRight[i] = accmRight[i+1]
		if i % 2 == 1 {
			accmRight[i] += remains[i+1] - remains[i]
		}
	}

	// fmt.Println(remains)
	// fmt.Println(accmLeft)
	// fmt.Println(accmRight)

	ans := BIGGEST
	for i := 0; i < k; i++ {
		ans = min(ans, accmLeft[i] + accmRight[i])
	}
