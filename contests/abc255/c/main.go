...
func solve() {
	...
	beforeXMax := calc(a, d, left)
	afterXMin := calc(a, d, right)

	ans := min(abs(beforeXMax - x), abs(afterXMin - x))
	fmt.Println(ans)
}

func calc(a, d, i int) int {
	return a + d * i
}
