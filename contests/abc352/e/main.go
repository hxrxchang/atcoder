...
func solve() {
	...
	for i := 0; i < m; i++ {
		...
		infos[i] = Info{k, c, nodes}
	}

	// costの重みが小さい順に並べる
	sort.Slice(infos, func(i, j int) bool {
		return infos[i].cost < infos[j].cost
	})
	...
	// 重みが小さい辺から順につなげていく
	for i := 0; i < m; i++ {
		for j := 0; j < infos[i].k - 1; j++ {
			nodes := infos[i].nodes
			// 全体のKの総和が`4× 10**5` なので全探索できる
			if !uf.isSame(nodes[j], nodes[j + 1]) {
				uf.unit(nodes[j], nodes[j + 1])
				ans += infos[i].cost
			}
		}
	}
