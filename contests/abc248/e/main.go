...
func solve() {
	...
	for a := 0; a < n; a++ {
		for b := a + 1; b < n; b++ {
			...
			for j := 0; j < len(pointsOnSameLine); j++ {
				for k := j + 1; k < len(pointsOnSameLine); k++ {
					seen[pointsOnSameLine[j]][pointsOnSameLine[k]] = true
				}
			}
			if len(pointsOnSameLine) >= k {
				ans += 1
			}
		}
	}
