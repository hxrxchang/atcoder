...
func solve() {
	...
	ans := BIGGEST
	permutationH := rangeSlice(h)
	for {
		permutationW := rangeSlice(w)
		for {
			gridC := make([][]int, h)
			for i := 0; i < h; i++ {
				gridC[i] = make([]int, w)
				for j := 0; j < w; j++ {
					gridC[i][j] = gridA[permutationH[i]][permutationW[j]]
				}
			}

			if checkIsSame(gridC, gridB) {
				ans = min(ans, calcMoveCnt(permutationH) + calcMoveCnt(permutationW))
			}

			if b := nextPermutation(sort.IntSlice(permutationW)); !b {
				break
			}
		}
		if b := nextPermutation(sort.IntSlice(permutationH)); !b {
			break
		}
	}
...
func checkIsSame(gridA, gridB [][]int) bool {
	for i := 0; i < len(gridA); i++ {
		for j := 0; j < len(gridA[i]); j++ {
