...
func solve() {
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if grid[i][j] == "#" || isStopItem(i, j, grid) {
				continue
			}
			if i - 1 >= 0 && grid[i-1][j] == "." && !isStopItem(i-1, j, grid) {
				uf.unit(to1D(i, j, w), to1D(i-1, j, w))
			}
			if i + 1 < h && grid[i+1][j] == "." && !isStopItem(i+1, j, grid) {
				uf.unit(to1D(i, j, w), to1D(i+1, j, w))
			}
			if j - 1 >= 0 && grid[i][j-1] == "." && !isStopItem(i, j-1, grid) {
				uf.unit(to1D(i, j, w), to1D(i, j-1, w))
			}
			if j + 1 < w && grid[i][j+1] == "." && !isStopItem(i, j+1, grid) {
				uf.unit(to1D(i, j, w), to1D(i, j+1, w))
			}
		}
	}
	...
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if isStopItem(i, j, grid) {
				st := newSet[int]()
				if i - 1 >= 0 && grid[i-1][j] != "#" && !isStopItem(i-1, j, grid) {
					st.Add(uf.root(to1D(i-1, j, w)))
				}
				if i + 1 < h && grid[i+1][j] != "#" && !isStopItem(i+1, j, grid) {
					st.Add(uf.root(to1D(i+1, j, w)))
				}
				if j - 1 >= 0 && grid[i][j-1] != "#" && !isStopItem(i, j-1, grid) {
					st.Add(uf.root(to1D(i, j-1, w)))
				}
				if j + 1 < w && grid[i][j+1] != "#" && !isStopItem(i, j+1, grid) {
					st.Add(uf.root(to1D(i, j+1, w)))
				}
...
func isStopItem(y, x int, grid [][]string) bool {
	if y - 1 >= 0 && grid[y-1][x] == "#" {
		return true
	}
...
func to1D(row, col, cols int) int {
    return row * cols + col
}
