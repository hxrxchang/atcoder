...
func solve() {
	...
	type City struct {
		idx, pref, birthYear, idxInPref int
	}

	pref := make(map[int][]City)
	for i := 0; i < m; i++ {
		in := getInts()
		city := City{i, in[0], in[1], -1}
		pref[in[0]] = append(pref[in[0]], city)
	}

	cities := make([]City, 0)
	for i := 1; i <= n; i++ {
		sort.Slice(pref[i], func(j, k int) bool {
			return pref[i][j].birthYear < pref[i][k].birthYear
		})
		for j := 0; j < len(pref[i]); j++ {
			pref[i][j].idxInPref = j+1
			cities = append(cities, pref[i][j])
		}
	}

	sort.Slice(cities, func(i, j int) bool {
		return cities[i].idx < cities[j].idx
	})

	for _, city := range cities {
		prefStr := withZero(i2s(city.pref))
		idxInPrefStr := withZero(i2s(city.idxInPref))
		fmt.Println(prefStr + idxInPrefStr)
	}
}

func withZero(s string) string {
	if len(s) >= 6 {
		return s
	}
