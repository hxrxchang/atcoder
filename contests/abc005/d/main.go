...
func solve() {
	...
	valByArea := make([]int, n*n+1)
	for i := 1; i <= n; i++ {
		for j := 1; j <= n; j++ {
			for l := i; l <= n; l++ {
				for m := j; m <= n; m++ {
					area := (l - i + 1) * (m - j + 1)
					val := accm[l][m] - accm[i-1][m] - accm[l][j-1] + accm[i-1][j-1]
					valByArea[area] = max(valByArea[area], val)
				}
			}
		}
	}
