...
func solve() {
	...
	for i := 0; i < n; i++ {
		...
		comparisons := []string{}
		if i == 0 {
			comparisons = append(comparisons, ss[i+1].value)
		} else if i == n-1 {
			comparisons = append(comparisons, ss[i-1].value)
		} else {
			comparisons = append(comparisons, ss[i-1].value, ss[i+1].value)
		}
		length := LongestPrefixMatch(target.value, comparisons)
		ans[target.idx] = length
	}
...
func LongestPrefixMatch(target string, comparisons []string) int {
    maxLength := 0

    for _, comparison := range comparisons {
        length := 0
        for i := 0; i < len(target) && i < len(comparison); i++ {
