...
func solve() {
	...
	for i := 0; i < m; i++ {
		in := getInts()
		u, v, w := in[0]-1, in[1]-1, in[2]
		graph[u] = append(graph[u], NextNode{v, w})
		graph[v] = append(graph[v], NextNode{u, w})
	}

	ans := BIGGEST

	var dfs func(from int, visited []bool, xor int)
	dfs = func(from int, visited []bool, xor int) {
		if from == n - 1 {
			ans = min(ans, xor)
		}
		for _, next := range graph[from] {
			if !visited[next.to] {
				...
				dfs(next.to, visited, xor ^ next.weight)
