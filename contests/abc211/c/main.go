...
func solve() {
	chokudai := strToSlice("chokudai", "")
	s := strToSlice(getStr(), "")

	// dp[i][j]: sのi文字目までを使って、chokudaiのj文字目まで作れる通り数
	dp := make([][]int, len(s) + 1)
	for i := 0; i < len(dp); i++ {
		dp[i] = make([]int, len(chokudai))
	}

	for i := 0; i < len(s); i++ {
		for j := 0; j < len(chokudai); j++ {
			if s[i] == chokudai[j] {
				if j == 0 {
					dp[i+1][j] = dp[i][j] + 1
				} else {
	...
	fmt.Println(dp[len(s)][len(chokudai) - 1])
}
