...
func solve() {
	...
	for i := 0; i < m; i++ {
		in := getInts()
		r, c := in[0], in[1]
		if st.Has(Position{r, c}) || st.Has(Position{r+1, c}) || st.Has(Position{r, c+1}) || st.Has(Position{r+1, c+1}) {
			continue
		}
		st.Add(Position{r, c})
		st.Add(Position{r+1, c})
		st.Add(Position{r, c+1})
		st.Add(Position{r+1, c+1})
		cnt += 1
	}
