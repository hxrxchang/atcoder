...
func solve() {
	...
	grid[0][(n-1)/2] = 1
	prevPos := Position{0, (n-1)/2}
	for i := 0; i < pow(n, 2) - 1; i++ {
		prevVal := grid[prevPos.y][prevPos.x]
		if grid[mod(prevPos.y - 1, n)][mod(prevPos.x + 1, n)] == 0 {
			grid[mod(prevPos.y - 1, n)][mod(prevPos.x + 1, n)] = prevVal + 1
			prevPos = Position{mod(prevPos.y - 1, n), mod(prevPos.x + 1, n)}
		} else {
			grid[mod(prevPos.y + 1, n)][prevPos.x] = prevVal + 1
			prevPos = Position{mod(prevPos.y + 1, n), prevPos.x}
		}
	}
