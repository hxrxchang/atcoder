...
func solve() {
	...
	// S[i]を見る時、S[i - 1]まででそれぞれの文字が何回出現しているかをカウント
	strCnt := map[string]int{}

	cnt := 0
	// i, jの組み合わせを考えていく。
	// jを先に決めて、それより前のiをどうするかを考える。
	for len, j := range s {
		// iとして採用できるのは、自分より前に出現している、自分とは違う文字のみ
		// なので、jまでの文字数からjと同じ文字の出現回数を引いたものを足す
		cnt += len - strCnt[j]
		strCnt[j]++
	}
	...
	// 同じ文字が2個以上ある場合は、元のSと同じものを作れるため、その分1を足す
	if maxCnt > 1 {
		cnt++
	}
