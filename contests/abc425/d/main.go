...
func solve() {
		...
		for x := 0; x < w; x++ {
			if grid[y][x] == "#" {
				for _, next := range []Position{{y-1, x}, {y+1, x}, {y, x-1}, {y, x+1}} {
					...
					que.PushBack(Position{next.y, next.x})
	...
	for que.Size() > 0 {
		// 今回の操作で黒になる点
		blackPositions := make([]Position, 0)
		// 全部取り出す
		for que.Size() > 0 {
			tmp := que.PopFront()
			y, x := tmp.y, tmp.x
			...
			if cnt == 1 {
				blackPositions = append(blackPositions, tmp)
			}
		}

		for _, black := range blackPositions {
			y, x := black.y, black.x
			grid[y][x] = "#"
			for _, next := range []Position{{y-1, x}, {y+1, x}, {y, x-1}, {y, x+1}} {
				if next.y < 0 || next.y >= h || next.x < 0 || next.x >= w {
