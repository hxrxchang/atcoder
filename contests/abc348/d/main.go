...
func solve() {
	...
	for i := 0; i < n; i++ {
		...
		que.PushBack(BfsItem{cell: drugPosition, energy: energy})
		dist[drugPosition.y][drugPosition.x] = true
		...
		for que.Size() > 0 {
			...
			for _, next := range []Cell{{cell.y + 1, cell.x}, {cell.y - 1, cell.x}, {cell.y, cell.x + 1}, {cell.y, cell.x - 1}} {
				...
				if energy > 0 {
					dist[next.y][next.x] = true
					que.PushBack(BfsItem{cell: next, energy: energy - 1})
				}
			}
		}
		...
		for i := 0; i < h; i++ {
			for j := 0; j < w; j++ {
				if dist[i][j] {
					graph[drugPosition] = append(graph[drugPosition], Cell{i, j})
				}
			}
		}
	}
