...
func solve() {
	...
	for {
		...
		for i := 0; i < n; i++ {
			for j := i+1; j < n; j++ {
				u, v := gIdxes[i], gIdxes[j]
				if u > v {
					u, v = v, u
				}
				if edgesG.Has(Edge{u, v}) != edgesH.Has(Edge{i, j}) {
					sum += cost[u][v]
				}
			}
		}
		ans = min(ans, sum)
		if !nextPermutation(sort.IntSlice(gIdxes)) {
			break
		}
	}
