...
// tuple heap
type TupleHeapItem[T constraints.Ordered, V any] struct {
	priority []T
	value    V
}

type TupleHeap[T constraints.Ordered, V any] []TupleHeapItem[T, V]
func (th TupleHeap[T, V]) Len() int {
	return len(th)
}
func (th TupleHeap[T, V]) Less(i, j int) bool {
	return th.compareSlices(i, j, 0)
}
func (th TupleHeap[T, V]) Swap(i, j int) {
	th[i], th[j] = th[j], th[i]
}
func (th *TupleHeap[T, V]) Push(x any) {
	*th = append(*th, x.(TupleHeapItem[T, V]))
}
func (h *TupleHeap[T, V]) Pop() interface{} {
	old := *h
	n := len(old)
	x := old[n-1]
	*h = old[0 : n-1]
...
func (th TupleHeap[T, V]) compareSlices(i, j, idx int) bool {
	a := th[i].priority
	b := th[j].priority
...
type MyTupleHeap[T constraints.Ordered, V any] struct {
	heap TupleHeap[T, V]
}
func newMyTupleHeap[T constraints.Ordered, V any]() *MyTupleHeap[T, V] {
	myTupleHeap := &MyTupleHeap[T, V]{}
	heap.Init(&myTupleHeap.heap)
	return myTupleHeap
}
func (h *MyTupleHeap[T, V]) Push(priority []T, value V) {
	heap.Push(&h.heap, TupleHeapItem[T, V]{
		priority: append([]T(nil), priority...),
		value:    value,
	})
}
func (h *MyTupleHeap[T, V]) Pop() TupleHeapItem[T, V] {
	return heap.Pop(&h.heap).(TupleHeapItem[T, V])
}
func (h *MyTupleHeap[T, V]) Len() int {
		return h.heap.Len()
}
