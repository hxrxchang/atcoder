...
func solve() {
	...
	ans := BIGGEST
	for i := 0; i < n; i++ {
		idx := bisectLeft(b, a[i])
		if idx <= 1 { // bの左端の場合、b[0]は番兵なので無視
			ans = min(ans, abs(a[i] - b[idx]), abs(a[i] - b[idx+1]))
		} else if idx >= m + 1 { // bの右端の場合、b[m+1]は番兵なので無視
			ans = min(ans, abs(a[i] - b[idx]), abs(a[i] - b[idx-1]))
		} else {
			// idx-1, idx, idx+1の3つの値のうち最小のものを選ぶ
			ans = min(ans, abs(a[i] - b[idx-1]), abs(a[i] - b[idx]), abs(a[i] - b[idx+1]))
		}
	}
