...
func getNexts(y, x, h, w int) []Next {
	nexts := []Next{}
	if y - 1 >= 0 {
		nexts = append(nexts, Next{y: y - 1, x: x, direction: "U"})
	}
	if y + 1 < h {
		nexts = append(nexts, Next{y: y + 1, x: x, direction: "D"})
	}
	if x - 1 >= 0 {
		nexts = append(nexts, Next{y: y, x: x - 1, direction: "L"})
	}
	if x + 1 < w {
		nexts = append(nexts, Next{y: y, x: x + 1, direction: "R"})
	}
	return nexts
}
