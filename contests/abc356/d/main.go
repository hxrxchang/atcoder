...
func solve() {
	...
	// bitを文字列スライスで表す: 例) 5 -> ["1", "0", "1"]
	bitsM := i2bit(m)
	// 逆順にする
	bitsM = reverse(bitsM)

	cnt := 0
	for i := 0; i < len(bitsM); i++ {
		if bitsM[i] == "1" {
			// 連続する部分1つ分の長さ
			seqLength := pow(2, i+1)
			// 連続する部分が何個あるか
			seqCnt := n / seqLength
			// nまでの数字の中で、ibit目が立っている数の個数
			onCnt := seqCnt * seqLength / 2
			cnt += onCnt
			cnt %= MOD

			// 余った部分で立っている数字があったら足す
			if n % seqLength >= seqLength / 2 {
				cnt += n % seqLength - seqLength / 2 + 1
				cnt %= MOD
			}
		}
	}
