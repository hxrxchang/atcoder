...
func solve() {
	...
	edges := make([][]int, 0, m-1)
	for i := 1; i < m; i++ {
		in := getInts()
		edges = append(edges, []int{in[0]-1, in[1]-1})
	}
	...
	for i, e := range edges {
		...
		uRoot := uf.root(u)
		vRoot := uf.root(v)
		if (uRoot == uf.root(leaderA) && vRoot == uf.root(leaderB)) || (uRoot == uf.root(leaderB) && vRoot == uf.root(leaderA)) {
			ans += modPow(2, m-i, MOD)
			ans %= MOD
		} else {
			uf.unit(u, v)
		}
