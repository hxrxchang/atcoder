...
func solve() {
	...
	if sliceToStr(s2, "") != sliceToStr(t2, "") {
		fmt.Println(-1)
		return
	}

	blocksS := make([]int, 0, len(s))
	block := 0
	for _, v := range s {
		if v == "A" {
			block++
		} else {
			blocksS = append(blocksS, block)
			block = 0
		}
	}
	if block != 0 {
	...
	block = 0
	blocksT := make([]int, 0, len(t))
	for _, v := range t {
		if v == "A" {
			block++
		} else {
			blocksT = append(blocksT, block)
			block = 0
		}
	}
	if block != 0 {
	...
	ans := 0
	for i := 0; i < max(len(blocksS), len(blocksT)); i++ {
		if i >= len(blocksS) || i >= len(blocksT) {
			if len(blocksS) > len(blocksT) {
				ans += blocksS[i]
			} else {
				ans += blocksT[i]
			}
		} else {
			ans += abs(blocksS[i] - blocksT[i])
		}
	}
