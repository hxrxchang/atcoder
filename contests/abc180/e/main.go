...
func solve() {
	...
	allVisited := (1 << n) - 1
	for from := 0; from < n; from++ {
		if dp[allVisited][from] == BIGGEST {
			continue
		}
		cost := abs(cities[from].x-cities[0].x) + abs(cities[from].y-cities[0].y) + max(0, cities[0].z-cities[from].z)
		ans = min(ans, dp[allVisited][from]+cost)
	}
