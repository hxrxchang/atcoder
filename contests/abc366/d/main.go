...
func solve() {
	...
	prefixSums := make([]*PrefixSum2D, n)
	for i := 0; i < n; i++ {
		grid := make([][]int, n)
		for j := 0; j < n; j++ {
			grid[j] = getInts()
		}
		prefixSums[i] = NewPrefixSum2D(grid)
	}

	q := getInt()
	for i := 0; i < q; i++ {
		in := getInts()
		x1, x2, y1, y2, z1, z2 := in[0]-1, in[1]-1, in[2]-1, in[3]-1, in[4]-1, in[5]-1

		cnt := 0
		for i := x1; i <= x2; i++ {
			prefixSum := prefixSums[i]
			cnt += prefixSum.Query(y1, z1, y2, z2)
		}
