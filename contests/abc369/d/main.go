...
func solve() {
    ...
    // dp[i][j]: i番目までのモンスターをj=0:奇数回で倒さない, j=1:奇数回で倒す j=2:偶数回で倒さない、j=3:偶数階で倒す、で得られる最大のポイント
    dp := make([][]int, n + 1)
    for i := 0; i <= n; i++ {
        dp[i] = make([]int, 4)
    }

    dp[0][0] = math.MinInt64
    ...
    for i := 1; i <= n; i++ {
        ...
        dp[i][3] = max(dp[i - 1][0], dp[i - 1][1]) + 2 * a[i - 1]
    }

    fmt.Println(max(dp[n][0], dp[n][1], dp[n][2], dp[n][3]))
}
