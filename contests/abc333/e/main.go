...
func solve() {
	...
	for i := 0; i < n; i++ {
		in := getInts()
		events[i] = Event{in[0], in[1]}
	}

	reversedEvents := reverse(events)
	monstersCnt := make(map[int]int)
	pickedPotions := newSet[int]()
	...
	for i, event := range reversedEvents {
		...
		if eventType == 2 {
			...
			// ポーションを使う
			if monstersCnt[value] > 0 {
				monstersCnt[value]--
				pickedPotions.Add(n - i - 1)
			}
		}
	}

	for _, cnt := range monstersCnt {
		if cnt > 0 {
			fmt.Println(-1)
			return
		}
