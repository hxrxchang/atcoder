...
func solve() {
	n := getInt()
	valsByDigits := make([]*Set[int], 11)

	for i := 0; i <= 10; i++ {
		valsByDigits[i] = newSet[int]()
	}

	for i := 0; i <= 30; i++ {
		v := pow(2, i)
		length := len(i2s(v))
		valsByDigits[length].Add(v)
	}

	for d := 2; d < 10; d++ {
		for rightLen := 1; rightLen <= d/2; rightLen++ {
			leftLen := d - rightLen
			for _, a := range valsByDigits[leftLen].Values() {
				for _, b := range valsByDigits[rightLen].Values() {
					valsByDigits[d].Add(s2i(i2s(a) + i2s(b)))
					valsByDigits[d].Add(s2i(i2s(b) + i2s(a)))
				}
			}
		}
	}
	...
	vals := make([]int, 0)
	for i := 0; i <= 10; i++ {
		vals = append(vals, valsByDigits[i].Values()...)
	}
	vals = sortSlice(vals)
	fmt.Println(vals[n-1])
}
