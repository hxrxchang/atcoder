...
func solve() {
	in := getInts()
	n, m, l, s, t := in[0], in[1], in[2], in[3], in[4]

	type NextNode struct {
		node, cost int
	}
	graph := make([][]NextNode, n)

	for i := 0; i < m; i++ {
		in := getInts()
		u, v, c := in[0]-1, in[1]-1, in[2]
		graph[u] = append(graph[u], NextNode{v, c})
	}
	...
	que.PushBack(QueItem{0, 0, 0})

	costs := make([][]int, n)
	for que.Size() > 0 {
		v := que.PopFront()
		for _, next := range graph[v.node] {
			if v.dist < l {
				if v.dist == l-1 {
					costs[next.node] = append(costs[next.node], v.cost+next.cost)
				}
				que.PushBack(QueItem{next.node, v.dist+1, v.cost+next.cost})
			}
		}
	}

	ans := make([]int, 0)
	for i, values := range costs {
