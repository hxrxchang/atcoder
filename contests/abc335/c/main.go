...
func solve() {
	...
	tracking := make([]Point, 0)
	for i := n; i > 0; i-- {
		tracking = append(tracking, Point{i, 0})
	}

	for i := 0; i < q; i++ {
		query := getStrs()
		if query[0] == "1" {
			latest := tracking[len(tracking) - 1]
			var next Point
			if query[1] == "R" {
				next = Point{latest.x + 1, latest.y}
			} else if query[1] == "L" {
				next = Point{latest.x - 1, latest.y}
			} else if query[1] == "U" {
				next = Point{latest.x, latest.y + 1}
			} else if query[1] == "D" {
				next = Point{latest.x, latest.y - 1}
			}
			tracking = append(tracking, next)
		} else {
			p := s2i(query[1])
			x, y := tracking[len(tracking) - p].x, tracking[len(tracking) - p].y
			fmt.Println(x, y)
		}
	}
}
