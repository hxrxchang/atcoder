...
func solve() {
	...
	komas := make([]Koma, m)
	komaCanGoSet := newSet[Koma]()
	for i := 0; i < m; i++ {
		in := getInts()
		a, b := in[0], in[1]
		komas[i] = Koma{a, b}
		komaCanGoSet.Add(Koma{a, b})
	}

	for i := 0; i < m; i++ {
		koma := komas[i]
		y, x := koma.y, koma.x
		candidats := []Koma{{y - 2, x - 1}, {y - 2, x + 1}, {y - 1, x - 2}, {y - 1, x + 2}, {y + 1, x - 2}, {y + 1, x + 2}, {y + 2, x - 1}, {y + 2, x + 1}}
		for _, candidate := range candidats {
			y2, x2 := candidate.y, candidate.x
			if y2 > 0 && y2 <= n && x2 > 0 && x2 <= n {
				komaCanGoSet.Add(candidate)
			}
		}
	}
	ans := n * n - komaCanGoSet.Size()
	fmt.Println(ans)
}
