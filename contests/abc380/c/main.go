...
func solve() {
	...
	oneBlocksIdx := make([]int, 0)

	tmp := make([]string, 0)
	for i := 0; i < n; i++ {
		if len(tmp) > 0 && tmp[len(tmp)-1] != s[i] {
	...
	for i, b := range blocks {
		if b[0] == "1" {
			oneBlocksIdx = append(oneBlocksIdx, i)
		}
	}

	ans := make([]string, n)

	targetOneBlocksIdx := oneBlocksIdx[k-1]
	targetOneBlockIdxBefore := oneBlocksIdx[k-2]
	tmpIdx := 0
	for i := 0; i < len(blocks); i++ {
		if i == targetOneBlocksIdx {
			continue
		}
		...
		if i == targetOneBlockIdxBefore {
			for _, v := range blocks[targetOneBlocksIdx] {
				ans[tmpIdx] = v
				tmpIdx++
			}
		}
