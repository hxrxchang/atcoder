...
func solve() {
	...
	for i := 0; i < n; i++ {
		if s[i] == "1" {
			oneIdxes = append(oneIdxes, i)
		}
	}
	center := len(oneIdxes)/2

	heap := newMyTupleHeap[int]()
	for i := 0; i < len(oneIdxes); i++ {
		if i == center {
			continue
		}
		// item[0]: 中心からの距離, item[1]: sの中のidx
		item := []int{abs(oneIdxes[i] - oneIdxes[center]) - 1, oneIdxes[i]}
		heap.push(item)
	}
